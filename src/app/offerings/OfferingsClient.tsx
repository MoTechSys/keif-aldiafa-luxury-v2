      ...
      <div className="container mx-auto px-4">
        ...
        <header className="mb-8 text-center">
          ...
          <motion.p
            ...
            className="text-[#F5F5DC]/70 max-w-2xl mx-auto"
          >
          ...
          {categories.map((cat) => (
            <button
              ...
              className={`px-6 py-3 rounded-full text-sm transition-all duration-300 flex items-center gap-2 ${
                activeTab === cat.id
                  ? "bg-[#B8860B] text-[#0f0f0f] font-bold shadow-lg shadow-[#B8860B]/20"
                  : "bg-[#1a1a1a] text-[#F5F5DC]/60 hover:bg-[#242424] border border-white/5"
              }`}
            >
              <span>{cat.icon}</span>
              {cat.label}
        ...
        >
          {currentCategory.items.map((item, idx) => (
            <motion.div
              ...
              className="group relative aspect-[3/4] rounded-2xl overflow-hidden cursor-pointer border border-white/5 shadow-xl"
            >
              <ImageWithFallback
                src={item.img}
                alt={item.name}
              ...
              <div className="absolute bottom-0 left-0 right-0 p-4 text-right">
                <h3 className="text-lg font-amiri text-[#D4A017] mb-1 group-hover:text-[#FFD700] transition-colors">
                  {item.name}
                </h3>
                <p className="text-xs text-[#F5F5DC]/70 line-clamp-1">
                  {item.description}
                </p>
              </div>
            </motion.div>
        ...
        <div className="mt-20 text-center">
          <a
            ...
            className="inline-flex items-center gap-3 px-8 py-4 rounded-full bg-gradient-to-r from-[#B8860B] to-[#D4A017] text-[#0f0f0f] font-bold hover:scale-105 transition-transform shadow-xl"
          >
            <span>اطلب لفعاليتك الآن</span>
